...
const ORDER_PROGRESS_BAR_WIDTH = 142;
const MAIN_ORDER_MILESTONES = MATH_CONFIG.mainOrderMilestones || {
  Easy: [
    { at: 0.55, kind: "shift", title: "CANDY SHIFT", detail: "3 candies change toward this order" }
  ],
  Medium: [
    { at: 0.38, kind: "shift", title: "CANDY SHIFT", detail: "3 candies change toward this order" },
    { at: 0.72, kind: "stripe", title: "STRIPE READY", detail: "A striped candy joins the board" }
  ],
  Hard: [
    { at: 0.28, kind: "shift", title: "CANDY SHIFT", detail: "4 candies change toward this order" },
    { at: 0.55, kind: "stripe", title: "STRIPE READY", detail: "A striped candy joins the board" },
    { at: 0.8, kind: "bomb", title: "BOMB READY", detail: "A bomb candy joins the board" }
  ]
};
const LABELS = {
  red: "Red",
...
class CandyOrdersScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.bonusEventSessionCounts = { special: 0, discount: 0, gold: 0, clear: 0, boardGold: 0 };
    this.goldenSpawnedThisMove = 0;
    this.goldenRevealedThisMove = 0;
  ...
  showOrderAlmostFx(index) {
    ...
    const banner = this.add.rectangle(W / 2, 250, W - 92, 34, 0x351352, 0.88)
      .setStrokeStyle(3, 0xfff06a, 0.95)
      .setDepth(120);
    banner.isFxSprite = true;
    this.fxSprites.add(banner);
    const label = this.add.text(W / 2, 250, "ORDER NEARLY READY", {
      fontSize: 18,
      fontStyle: "900",
      color: "#fff06a",
      stroke: "#7a2d93",
    ...
    for (let i = 0; i < 2; i++) this.time.delayedCall(i * 160, () => this.burstAt(W / 2, y, 0xfff06a));
  }
  ...
  showOrderRewardSummary(completions, totalReward) {
    ...
    const upgradedCompletions = coinCompletions.filter((item) => Number(item.upgradeMult || 1) > 1);
    const hasUpgrade = upgradedCompletions.length > 0;
    const boostText = coinCompletions
      ...
      .join("  ");
    const baseTotalMult = coinCompletions.reduce((sum, item) => sum + Number(item.baseRollMult ?? item.rollMult), 0);
    ...
    const panel = this.add.rectangle(W / 2, 430, W - 44, separateDraws ? 248 : 232, 0x4a1a6b, 0.97).setStrokeStyle(5, 0xfff06a, 1).setDepth(summaryDepth + 1);
    ...
    const multiplier = this.add.text(W / 2, separateDraws ? 468 : 430, separateDraws ? "TOTAL +0x" : "+0x", {
    ...
    const upgradeBadge = this.add.text(W / 2, 394, "", {
      fontSize: 19,
      fontStyle: "900",
      color: "#fff06a",
      stroke: "#7a2d93",
      strokeThickness: 6
    }).setOrigin(0.5).setDepth(summaryDepth + 5);
    const rangeText = this.add.text(W / 2, separateDraws ? 497 : 470, `${separateDraws ? `${coinCompletions.length} SEPARATE DRAWS` : (boostText || `RANGE ${rangeMin}-${rangeMax}x`)}${scatterCount ? `  +${scatterCount} SCATTER` : ""}`, {
    ...
    const rewardText = this.add.text(W / 2, separateDraws ? 521 : 505, `COINS +${totalReward}`, {
    ...
    const items = [veil, panel, topLine, title, upgradeBadge, multiplier, rangeText, rewardText, ...drawLabels, ...drawValues];
    items.forEach((item) => {
    ...
    let finalRevealAt = separateDraws ? 850 : 930;
    if (separateDraws) {
      coinCompletions.forEach((item, index) => {
        ...
        const baseRevealAt = 720 + index * 230;
        const upgraded = Number(item.upgradeMult || 1) > 1;
        this.time.delayedCall(baseRevealAt, () => {
          drawValues[index].setText(`+${item.baseRollMult ?? item.rollMult}x`).setScale(1.22);
        ...
        if (upgraded) {
          const upgradeRevealAt = baseRevealAt + 360;
          finalRevealAt = Math.max(finalRevealAt, upgradeRevealAt);
          this.time.delayedCall(upgradeRevealAt, () => {
            const superUpgrade = item.upgradeKind === "super";
            drawLabels[index]
              .setText(`${String(item.order?.tier || `ORDER ${index + 1}`).toUpperCase()}  x${item.upgradeMult}`)
              .setFontSize(10)
              .setColor(superUpgrade ? "#8ee8ff" : "#fff06a");
            drawValues[index]
              .setText(`+${item.rollMult}x`)
              .setColor(superUpgrade ? "#8ee8ff" : "#fff06a")
              .setScale(1.38);
            this.tweens.add({ targets: drawValues[index], scale: 1, duration: 360, ease: "Back.Out" });
            this.playUnlockSound();
            this.cameras.main.shake(superUpgrade ? 260 : 150, superUpgrade ? 0.01 : 0.005);
            this.burstAt(drawValues[index].x, drawValues[index].y, superUpgrade ? 0x8ee8ff : 0xfff06a);
          });
        }
      });
    } else {
    ...
    if (separateDraws) finalRevealAt = Math.max(finalRevealAt, 850 + (coinCompletions.length - 1) * 230);
    this.time.delayedCall(finalRevealAt, () => {
      multiplier.setText(separateDraws ? `TOTAL +${totalMult}x` : `+${baseTotalMult}x`);
      this.playUnlockSound();
      this.cameras.main.shake(240, 0.008);
      this.burstAt(W / 2, multiplier.y, 0xfff06a);
    });
    if (!separateDraws && hasUpgrade) {
      const item = upgradedCompletions[0];
      const upgradeRevealAt = finalRevealAt + 420;
      finalRevealAt = upgradeRevealAt;
      this.time.delayedCall(upgradeRevealAt, () => {
        const superUpgrade = item.upgradeKind === "super";
        const accent = superUpgrade ? 0x8ee8ff : 0xfff06a;
        const accentCss = superUpgrade ? "#8ee8ff" : "#fff06a";
        panel.setFillStyle(superUpgrade ? 0x123b5c : 0x6b281a, 0.98).setStrokeStyle(7, accent, 1);
        topLine.setFillStyle(accent, 1);
        title.setText(superUpgrade ? "SUPER ORDER UPGRADE!" : "ORDER UPGRADE!").setColor(accentCss);
        upgradeBadge.setText(`PRIZE x${item.upgradeMult}`).setColor(accentCss).setAlpha(1).setScale(0.55);
        multiplier.setText(`+${item.rollMult}x`).setColor(accentCss).setScale(1.42);
        rangeText.setText(`BASE ${item.baseRollMult}x  ${item.upgradeLabel || "UPGRADE"}`).setColor(accentCss);
        this.tweens.add({ targets: [upgradeBadge, multiplier], scale: 1, duration: 420, ease: "Back.Out" });
        this.playUnlockSound();
        this.cameras.main.shake(superUpgrade ? 360 : 220, superUpgrade ? 0.014 : 0.008);
        for (let i = 0; i < (superUpgrade ? 6 : 4); i++) {
          this.time.delayedCall(i * 90, () => this.burstAt(W / 2 + Phaser.Math.Between(-110, 110), 420 + Phaser.Math.Between(-55, 55), accent));
        }
      });
    } else if (separateDraws && hasUpgrade) {
      this.time.delayedCall(finalRevealAt + 180, () => {
        rangeText.setText(`${upgradedCompletions.length} UPGRADE${upgradedCompletions.length > 1 ? "S" : ""}  /  ${coinCompletions.length} DRAWS`).setColor("#fff06a");
      });
    }
    const fadeDelay = separateDraws
      ? Math.max(2350, finalRevealAt + 1250)
      : hasUpgrade
        ? Math.max(this.autoPlayEnabled ? 2250 : 2850, finalRevealAt + 1100)
        : (this.autoPlayEnabled ? 1450 : 2100);
    this.tweens.add({
      targets: items,
      alpha: 0,
      delay: fadeDelay,
  ...
  createUi() {
    ...
    this.titleLogoArt = this.add.image(W / 2, 66, "ui-logo-candy-orders")
    ...
    for (let i = 0; i < 3; i++) {
      ...
      const progressTrack = this.add.rectangle(rowLayout.textX, rowCenter + rowLayout.progressDy, ORDER_PROGRESS_BAR_WIDTH, 9, 0x260713, 0.9)
        .setOrigin(0, 0.5)
        .setStrokeStyle(1, 0xffb6d2, 0.72)
        .setDepth(15);
      const progressFill = this.add.rectangle(rowLayout.textX + 1, rowCenter + rowLayout.progressDy, 1, 5, 0x45e1da, 1)
        .setOrigin(0, 0.5)
        .setDepth(16);
      const milestoneNodes = [];
      for (let nodeIndex = 0; nodeIndex < 3; nodeIndex++) {
        const node = this.add.circle(rowLayout.textX, rowCenter + rowLayout.progressDy, 4, 0x5a1020, 1)
          .setStrokeStyle(1.5, 0xfff06a, 1)
          .setDepth(19)
          .setVisible(false);
      ...
      this.orderRows.push({ glow, frameArt, panel, dotA, dotB, icon, label, progress, progressTrack, progressFill, milestoneNodes, rewardPlate, reward, keyRewardIcon, iconGroup: null, nearTweens: [], rowCenter, rowLayout });
    }
  ...
  showPreStart() {
    ...
    this.orderRows.forEach((row, i) => {
      ...
      row.progress.setPosition(row.progressTrack.x + ORDER_PROGRESS_BAR_WIDTH, row.rowCenter + row.rowLayout.labelDy);
      row.progressFillRatio = 0;
      row.progressFill.setDisplaySize(1, 5).setVisible(false);
      row.milestoneNodes.forEach(({ node }) => {
  ...
  setMode(mode) {
    ...
    this.orderRows.forEach((row) => {
      ...
      row.milestoneNodes.forEach(({ node }) => {
        node.setVisible((mode === "game" || isFree) && !isFree);
      });
      row.progressFill.setVisible((mode === "game" || isFree) && Number(row.progressFillRatio || 0) > 0);
    });
  ...
  orderMultRange(order) {
    const mainTickets = this.mainOrderPayoutTickets(order);
    if (mainTickets.length) {
      const ranges = mainTickets.map((ticket) => this.mainOrderTicketRange(order, ticket));
      const min = Math.min(...ranges.map((range) => range.min));
      const max = Math.max(...ranges.map((range) => range.max));
      return { min, median: this.effectiveMult(order.mult), max };
    }
  ...
  rollOrderMult(order) {
    ...
    const mainTickets = this.mainOrderPayoutTickets(order);
    if (mainTickets.length) {
      const ticket = this.pickWeightedBand(mainTickets);
      const selected = this.mainOrderTicketRange(order, ticket);
      return { range, mult: Phaser.Math.Between(selected.min, selected.max) };
    }
  ...
  mainOrderPayoutTickets(order) {
    if (!order || order.scope === "free") return [];
    const tickets = MATH_CONFIG.mainOrderPayoutTickets?.[order.tier];
    return Array.isArray(tickets) ? tickets.filter((ticket) => Number(ticket.weight) > 0) : [];
  }

  mainOrderTicketRange(order, ticket) {
    const median = this.effectiveMult(order.mult);
    const floor = Math.max(1, Number(MATH_CONFIG.mainOrderRewardFloorByTier?.[order.tier] || 1), Number(ticket.minAward || 0));
    const min = Math.max(floor, Math.round(median * Number(ticket.minFactor ?? 1)));
    const max = Math.max(min, Math.round(median * Number(ticket.maxFactor ?? ticket.minFactor ?? 1)));
    return { min, max };
  }

  rollMainOrderUpgrade(order) {
    if (!order || order.scope === "free") return { kind: "none", label: "", mult: 1 };
    const ticket = this.pickWeightedBand(MATH_CONFIG.mainOrderUpgradeTicketsByTier?.[order.tier] || MATH_CONFIG.mainOrderUpgradeTickets);
    if (!ticket || ticket.kind === "none") return { kind: "none", label: "", mult: 1 };
    const multipliers = Array.isArray(ticket.multipliers) && ticket.multipliers.length ? ticket.multipliers : [1];
    const mult = Number(Phaser.Utils.Array.GetRandom(multipliers)) || 1;
    return {
      kind: ticket.kind || "upgrade",
      label: ticket.label || "ORDER UPGRADE",
  ...
  createOrder(tier) {
    const tierIndex = ["Easy", "Medium", "Hard"].indexOf(tier);
    const mainPool = MATH_CONFIG.mainOrderPools[tierIndex];
    const template = this.pickWeightedBand(mainPool) || Phaser.Utils.Array.GetRandom(mainPool);
    const spec = { ...template };
    ...
    order.milestonesTriggered = [];
    order.milestoneShiftType = order.kind === "color" ? order.type : Phaser.Utils.Array.GetRandom(TYPES);
    return order;
  }
  ...
  createFreeOrder(tier) {
    ...
    const freePool = MATH_CONFIG.freeOrderPools[tierIndex];
    const template = this.pickWeightedBand(freePool) || Phaser.Utils.Array.GetRandom(freePool);
    const spec = { ...template };
  ...
  previewAutoMove(move) {
    ...
    const goldenHits = [...remove].filter((key) => {
      const [r, c] = key.split(",").map(Number);
      return !!this.board[r]?.[c]?.golden;
    }).length;
  ...
  destroyCandySprite(sprite) {
    if (!sprite) return;
    // Destroy events remove children from the group, so iterate a snapshot.
  ...
  createCandySprite(tile, r, c, instant = false, startY = null) {
    ...
    if (tile.golden && !tile.special && !tile.scatter && !tile.chest) {
      const glow = this.add.circle(x, y, this.cell * 0.50, 0xffc43d, 0.18)
        .setStrokeStyle(3, 0xfff3a0, 0.34)
        .setAlpha(0.16)
        .setBlendMode(Phaser.BlendModes.ADD);
      const goldRim = this.add.image(x, y, this.symbolKey(tile))
        .setTintFill(0xffbf22)
        .setAlpha(0.98)
        .setScale(img.scaleX * 1.13, img.scaleY * 1.13);
      ...
      glow.isGoldenLayer = true;
      goldRim.isGoldenLayer = true;
      star.isGoldenLayer = true;
      [glow, goldRim, star].forEach((child) => {
        child.isBoardSymbol = true;
        child.goldenBaseAlpha = child.alpha;
      });
      glow.setDepth(3.2);
      goldRim.setDepth(3.6);
      img.setDepth(4);
      star.setDepth(5);
      glow.goldenPulseTween = this.tweens.add({
        targets: glow,
      ...
      g.addMultiple([glow, goldRim, img, star, hit]);
    } else {
    ...
    g.getChildren().forEach((child) => {
      if (child === img || !child.isGoldenLayer) child.setDepth(4);
      child.row = r;
      child.col = c;
      if (!instant) {
  ...
  async performMove(r1, c1, r2, c2) {
    ...
    await this.maybeSpawnGoldenMoveEvent();
  ...
  goldenConfig() {
    return MATH_CONFIG.goldenCandy || {};
  }

  countGoldenCandies() {
    return this.board.flat().filter((tile) => tile?.golden && !tile.special && !tile.scatter && !tile.chest).length;
  }

  rollGoldenCount(tickets, fallbackMin = 1, fallbackMax = 2) {
    const ticket = this.pickWeightedBand(Array.isArray(tickets) ? tickets : []);
    return ticket
      ? this.rollIntegerRange(ticket, fallbackMin)
      : Phaser.Math.Between(fallbackMin, fallbackMax);
  }

  rollGoldenReveal() {
    const ticket = this.pickWeightedBand(this.goldenConfig().revealTickets) || { kind: "stripe", label: "STRIPED" };
    if (ticket.kind === "bomb") return { kind: ticket.kind, label: ticket.label, special: "bomb", extraSpecials: 0, spread: 0 };
    if (ticket.kind === "chocolate") return { kind: ticket.kind, label: ticket.label, special: "chocolate", extraSpecials: 0, spread: 0 };
    if (ticket.kind === "double") return { kind: ticket.kind, label: ticket.label, special: "bomb", extraSpecials: 1, spread: 0 };
    if (ticket.kind === "spread") return { kind: ticket.kind, label: ticket.label, special: "chocolate", extraSpecials: 0, spread: Phaser.Math.Between(2, 4) };
    return {
      kind: "stripe",
      label: ticket.label || "STRIPED",
      special: Math.random() < 0.5 ? "stripeRow" : "stripeCol",
      extraSpecials: 0,
      spread: 0
    };
  }

  extractGoldenReveals(remove, create) {
    const createList = Array.isArray(create) ? create : [];
    const keys = new Set(remove);
    createList.forEach(({ r, c }) => keys.add(`${r},${c}`));
    const reveals = [];
    for (const key of keys) {
      const [r, c] = key.split(",").map(Number);
      const tile = this.board[r]?.[c];
      if (!tile?.golden || tile.special || tile.scatter || tile.chest) continue;
      remove.delete(key);
      for (let i = createList.length - 1; i >= 0; i--) {
        if (createList[i].r === r && createList[i].c === c) createList.splice(i, 1);
      }
      reveals.push({ r, c, type: tile.type, ...this.rollGoldenReveal() });
    }
    return reveals;
  }

  async playGoldenCrackFx(reveals) {
    if (!reveals.length) return;
    reveals.forEach(({ r, c }, index) => {
      const sprite = this.sprites[r]?.[c];
      if (sprite) {
        const visible = sprite.getChildren().filter((child) => child.alpha > 0.01);
        visible.forEach((child) => {
          this.tweens.add({
            targets: child,
            scaleX: child.scaleX * 1.18,
            scaleY: child.scaleY * 1.18,
            duration: 110,
            yoyo: true,
            repeat: 1,
            delay: index * 45,
      ...
      this.time.delayedCall(index * 45, () => this.burstAt(this.cellX(c), this.cellY(r), 0xffd23f));
    });
    this.playCoinSpraySound();
    if (reveals.length >= 3) {
      this.showBonusEventBanner(reveals.length >= 5 ? `GOLDEN RUSH x${reveals.length}` : `GOLDEN HIT x${reveals.length}`, 0xffdf57);
    }
    await this.wait(300 + Math.min(8, reveals.length) * 35);
  }

  async materializeGoldenReveals(reveals) {
    if (!reveals.length) return;
    let extraSpecials = 0;
    let spread = 0;
    for (const reveal of reveals) {
      ...
      if (this.gameMode === "free") {
        this.freeRemoved += 1;
        if (this.freeRemovedByColor[reveal.type] !== undefined) this.freeRemovedByColor[reveal.type] += 1;
      } else {
        this.totalRemoved += 1;
        if (this.removedByColor[reveal.type] !== undefined) this.removedByColor[reveal.type] += 1;
      }
      const oldSprite = this.sprites[r]?.[c];
      if (oldSprite) this.destroyCandySprite(oldSprite);
      this.board[r][c] = { type: reveal.type, special: reveal.special };
      this.sprites[r][c] = this.createCandySprite(this.board[r][c], r, c, false);
      extraSpecials += Number(reveal.extraSpecials || 0);
      spread += Number(reveal.spread || 0);
      const label = this.add.text(this.cellX(c), this.cellY(r) - this.cell * 0.40, reveal.label, {
        fontFamily: "Arial Black",
        fontSize: reveals.length > 2 ? "9px" : "11px",
        color: "#fff7bd",
        stroke: "#6b2d00",
        strokeThickness: 4
      ...
      this.burstAt(this.cellX(c), this.cellY(r), reveal.special === "chocolate" ? 0xffffff : 0xffd23f);
    }
    if (this.gameMode === "free") this.updateFreeUi();
    else this.updateOrders();
    this.goldenRevealedThisMove += reveals.length;
    const extras = Phaser.Utils.Array.Shuffle([...this.plainCandyPositions()]).slice(0, Math.min(4, extraSpecials));
    extras.forEach(([r, c]) => {
      const type = this.board[r][c].type;
      const special = Math.random() < 0.58 ? (Math.random() < 0.5 ? "stripeRow" : "stripeCol") : "bomb";
      this.replacePlainCandy(r, c, { type, special });
    });
    await this.wait(300 + Math.min(6, reveals.length) * 35);
    if (spread > 0) await this.spawnGoldenCandies(Math.min(8, spread), "spread");
  }

  async removePositions(remove, create) {
    const goldenReveals = this.extractGoldenReveals(remove, create);
    await this.playGoldenCrackFx(goldenReveals);
    const targets = [];
    ...
    await this.materializeGoldenReveals(goldenReveals);
    return goldenReveals;
  }
  ...
  async collectBonusScatterRetrigger() {
    ...
    if (MATH_CONFIG.freeOrdersRefreshOnRetrigger) {
      this.orders = this.orders.map((order) => (
        order.scope === "free" && order.completed ? this.createFreeOrder(order.tier) : order
      ));
      this.updateOrders();
    }
    this.updateFreeUi();
  ...
  async spawnGoldenCandies(requestedCount, source = "move") {
    const config = this.goldenConfig();
    const boardLimit = Math.max(1, Number(config.maxOnBoard || 12));
    const room = Math.max(0, boardLimit - this.countGoldenCandies());
    const positions = Phaser.Utils.Array.Shuffle([...this.plainCandyPositions()]);
    const count = Math.min(Math.max(0, Math.floor(requestedCount)), room, positions.length);
    if (count <= 0) return 0;
    const picked = positions.slice(0, count);
    picked.forEach(([r, c], index) => {
      const tile = this.board[r][c];
      tile.golden = true;
      const oldSprite = this.sprites[r]?.[c];
      if (oldSprite) this.destroyCandySprite(oldSprite);
      this.sprites[r][c] = this.createCandySprite(tile, r, c, true);
      const x = this.cellX(c);
      const y = this.cellY(r);
      const droplet = this.add.circle(x, this.boardY - 34 - index * 5, 5, 0xffdc55, 0.95)
        .setStrokeStyle(2, 0xffffff, 0.9)
        .setDepth(31)
        .setBlendMode(Phaser.BlendModes.ADD);
      droplet.isFxSprite = true;
      this.fxSprites.add(droplet);
      this.tweens.add({
        targets: droplet,
        y,
        scale: 1.55,
        ...
        ease: "Cubic.In",
        onComplete: () => {
          this.burstAt(x, y, 0xffd23f);
          this.destroyFx(droplet);
        }
      });
      const goldenLayers = this.sprites[r][c].getChildren().filter((child) => child.isGoldenLayer);
      goldenLayers.forEach((child) => {
        if (child.goldenPulseTween) child.goldenPulseTween.pause();
        child.setAlpha(0);
        this.tweens.add({
          targets: child,
          alpha: child.goldenBaseAlpha,
          delay: 180 + index * 22,
          duration: 180,
          ease: "Cubic.Out",
          onComplete: () => {
            if (child.active && child.goldenPulseTween) child.goldenPulseTween.restart();
          }
        });
      });
    });
    this.goldenSpawnedThisMove += count;
    this.playPopSound(count >= 5 ? 1520 : 1180);
    if (source === "bonus" || count >= 5) {
      const title = count >= 8 ? `GOLDEN RUSH x${count}` : count >= 5 ? `GOLDEN SHOWER x${count}` : `GOLDEN CANDY x${count}`;
      this.showBonusEventBanner(title, 0xffd23f);
      if (count >= 8) this.cameras.main.flash(240, 255, 221, 83);
    }
    await this.wait(340 + Math.min(count, 8) * 24);
    return count;
  }

  async maybeSpawnGoldenMoveEvent() {
    const config = this.goldenConfig();
    const rate = this.gameMode === "free"
      ? Number(config.freeMoveTriggerRate || 0)
      : Number(config.mainMoveTriggerRate || 0);
    if (rate <= 0 || Math.random() >= rate) return false;
    const count = this.rollGoldenCount(config.moveCountTickets, 1, 2);
    const spawned = await this.spawnGoldenCandies(count, "move");
    if (spawned > 0 && this.gameMode === "free") this.freeMoveHadEvent = true;
    return spawned > 0;
  }
  ...
  plainCandyPositions() {
    ...
    for (let r = 0; r < this.rows; r++) {
      for (let c = 0; c < COLS; c++) {
        const tile = this.board[r]?.[c];
        if (tile && !tile.scatter && !tile.chest && !tile.special && !tile.golden) positions.push([r, c]);
      }
    }
    return positions;
  }
  ...
  async bonusEventGoldenCandies() {
    const config = this.goldenConfig();
    const count = this.rollGoldenCount(config.bonusCountTickets || config.moveCountTickets, 2, 5);
    return (await this.spawnGoldenCandies(count, "bonus")) > 0;
  }
  ...
  async maybeTriggerBonusMoveEvent() {
    ...
    const runEvent = {
      ...
      boardGold: () => this.bonusEventGoldenCandies()
    };
  ...
  milestoneTargetType(order) {
    if (order?.kind === "color" && TYPES.includes(order.type)) return order.type;
    if (TYPES.includes(order?.milestoneShiftType)) return order.milestoneShiftType;
    const counts = Object.fromEntries(TYPES.map((type) => [type, 0]));
    this.board.flat().forEach((tile) => {
      if (tile && !tile.scatter && !tile.chest && !tile.special && TYPES.includes(tile.type)) counts[tile.type] += 1;
    });
    return [...TYPES].sort((a, b) => counts[b] - counts[a])[0] || Phaser.Utils.Array.GetRandom(TYPES);
  }

  async showOrderMilestoneCard(orderIndex, order, milestone) {
    const group = this.add.group();
    const depth = 146;
    const y = H / 2 - 8;
    const accent = milestone.kind === "golden" ? 0xffdf57 : milestone.kind === "bomb" ? 0xffd24a : milestone.kind === "stripe" ? 0xff86bd : 0x8ee8ff;
    const panelColor = milestone.kind === "golden" ? 0x70410b : milestone.kind === "bomb" ? 0x6a2b12 : milestone.kind === "stripe" ? 0x661d42 : 0x173b55;
    const veil = this.add.rectangle(W / 2, H / 2, W, H, 0x250712, 0.24).setDepth(depth);
    const panel = this.add.rectangle(W / 2, y, 318, 104, panelColor, 0.98)
      .setStrokeStyle(4, accent, 1)
      .setDepth(depth + 1);
    const iconKey = milestone.kind === "golden"
      ? `sym-${this.milestoneTargetType(order)}`
      : milestone.kind === "stripe"
      ? "sym-stripe-row"
      : milestone.kind === "bomb"
        ? "sym-bomb"
        : `sym-${this.milestoneTargetType(order)}`;
    const icon = this.add.image(W / 2 - 106, y, iconKey).setDepth(depth + 2);
    icon.setScale(44 / Math.max(icon.width, icon.height));
    let goldenHalo = null;
    let goldenStar = null;
    if (milestone.kind === "golden") {
      goldenHalo = this.add.circle(W / 2 - 106, y, 29, 0xffd23f, 0.26)
        .setStrokeStyle(3, 0xffffff, 0.78)
        .setDepth(depth + 1);
      goldenStar = this.add.star(W / 2 - 88, y - 18, 5, 5, 10, 0xffe35b, 1)
        .setStrokeStyle(2, 0xffffff, 0.92)
        .setDepth(depth + 3);
    }
    const title = this.add.text(W / 2 + 18, y - 17, milestone.title, {
      fontSize: 21,
      fontStyle: "900",
      color: "#fff06a",
      stroke: "#351352",
    ...
    const detail = this.add.text(W / 2 + 18, y + 18, milestone.detail, {
      fontSize: 11,
      fontStyle: "800",
      color: "#ffffff",
      stroke: "#351352",
    ...
    group.addMultiple([veil, panel, ...(goldenHalo ? [goldenHalo] : []), icon, ...(goldenStar ? [goldenStar] : []), title, detail]);
    const row = this.orderRows[orderIndex];
    if (row) this.tweens.add({ targets: [row.progressTrack, row.progressFill], scaleY: 1.7, duration: 140, yoyo: true, repeat: 2, ease: "Sine.InOut" });
    panel.setScale(0.76);
    icon.setScale(icon.scaleX * 0.55);
    title.setScale(0.78);
    detail.setAlpha(0);
    this.tweens.add({ targets: [panel, title], scale: 1, duration: 240, ease: "Back.Out" });
    this.tweens.add({ targets: icon, scale: 44 / Math.max(icon.width, icon.height), angle: 360, duration: 360, ease: "Back.Out" });
    this.tweens.add({ targets: detail, alpha: 1, duration: 180, delay: 180 });
    this.playUnlockSound();
    this.burstAt(W / 2 - 106, y, accent);
    await this.wait(700);
    this.tweens.add({ targets: group.getChildren(), alpha: 0, duration: 180, ease: "Cubic.In" });
    await this.wait(170);
    this.destroyTempGroup(group);
  }

  replacePlainCandy(r, c, tile) {
    const oldSprite = this.sprites[r]?.[c];
    if (oldSprite) this.destroyCandySprite(oldSprite);
    this.board[r][c] = tile;
    this.sprites[r][c] = this.createCandySprite(tile, r, c, true);
    const sprite = this.sprites[r][c];
    sprite.getChildren().forEach((child) => {
      const baseX = child.scaleX;
      const baseY = child.scaleY;
      child.setScale(baseX * 0.35, baseY * 0.35);
      this.tweens.add({ targets: child, scaleX: baseX, scaleY: baseY, duration: 280, ease: "Back.Out" });
    });
    this.burstAt(this.cellX(c), this.cellY(r), 0xfff06a);
  }

  async applyOrderMilestone(order, milestone) {
    const positions = Phaser.Utils.Array.Shuffle([...this.plainCandyPositions()]);
    if (!positions.length) return false;
    if (milestone.kind === "golden") {
      return (await this.spawnGoldenCandies(Math.max(1, Number(milestone.count || 1)), "checkpoint")) > 0;
    }
    if (milestone.kind === "shift") {
      const targetType = this.milestoneTargetType(order);
      const preferred = positions.filter(([r, c]) => this.board[r][c].type !== targetType);
      const count = Math.max(1, Number(milestone.count || (order.tier === "Hard" ? 4 : 3)));
      preferred.slice(0, count).forEach(([r, c]) => this.replacePlainCandy(r, c, { type: targetType, special: null }));
      this.playPopSound(1480);
      await this.wait(320);
      if (this.findMatches().length > 0) await this.resolveAll();
      return true;
    }

    const [r, c] = positions[0];
    const baseType = this.board[r][c].type;
    const special = milestone.kind === "bomb" ? "bomb" : (Math.random() < 0.5 ? "stripeRow" : "stripeCol");
    this.replacePlainCandy(r, c, { type: baseType, special });
    this.playPopSound(milestone.kind === "bomb" ? 980 : 1260);
    await this.wait(420);
    return true;
  }

  async triggerMainOrderMilestones() {
    if (this.gameMode !== "main") return false;
    let triggeredAny = false;
    let guard = 0;
    while (guard++ < 12) {
      let next = null;
      for (let orderIndex = 0; orderIndex < this.orders.length && !next; orderIndex++) {
        const order = this.orders[orderIndex];
        if (!order || order.scope === "free" || order.completed) continue;
        const progress = this.orderProgress(order);
        const plan = this.orderMilestonePlan(order);
        const triggered = new Set(order.milestonesTriggered || []);
        for (let milestoneIndex = 0; milestoneIndex < plan.length; milestoneIndex++) {
          const milestone = plan[milestoneIndex];
          const key = `${milestoneIndex}:${milestone.kind}`;
          if (!triggered.has(key) && progress >= Math.ceil(order.need * milestone.at)) {
            next = { order, orderIndex, milestone: this.resolveOrderMilestoneEvent(order, milestone), key };
            break;
          }
        }
      }
      if (!next) break;
      next.order.milestonesTriggered = [...(next.order.milestonesTriggered || []), next.key];
      this.updateOrders();
      await this.showOrderMilestoneCard(next.orderIndex, next.order, next.milestone);
      await this.applyOrderMilestone(next.order, next.milestone);
      triggeredAny = true;
      this.updateOrders();
    }
    return triggeredAny;
  ...
  resolveOrderMilestoneEvent(order, milestone) {
    const config = this.goldenConfig();
    const chance = Number(config.checkpointChanceByTier?.[order?.tier] || 0);
    if (chance <= 0 || Math.random() >= chance) return milestone;
    const range = config.checkpointCountByTier?.[order?.tier] || { min: 1, max: 2 };
    const count = this.rollIntegerRange(range, 1);
    return {
      ...milestone,
      kind: "golden",
      count,
      title: "GOLDEN CANDY",
      detail: `${count} golden ${count === 1 ? "candy joins" : "candies join"} the board`
    };
  }
  ...
  updateOrders() {
    this.orders.forEach((order, i) => {
      ...
      this.updateOrderProgressBar(row, order, progress, done, layout);
  ...
  orderMilestonePlan(order) {
    if (!order || order.scope === "free") return [];
    return MAIN_ORDER_MILESTONES[order.tier] || [];
  }

  updateOrderProgressBar(row, order, progress, done, layout) {
    const clampedProgress = Math.min(progress, order.need);
    const ratio = Phaser.Math.Clamp(clampedProgress / Math.max(1, order.need), 0, 1);
    const trackX = layout.textX;
    const trackY = row.rowCenter + layout.progressDy;
    const fillWidth = (ORDER_PROGRESS_BAR_WIDTH - 2) * ratio;
    row.progressTrack.setPosition(trackX, trackY).setDisplaySize(ORDER_PROGRESS_BAR_WIDTH, 9);
    row.progressFillRatio = ratio;
    row.progressFill.setPosition(trackX + 1, trackY)
      .setDisplaySize(Math.max(1, fillWidth), 5)
      .setVisible(this.currentUiMode === "game" && fillWidth > 0);
    row.progressFill.setFillStyle(done ? 0xfff06a : ratio >= 0.75 ? 0xff8fc7 : 0x45e1da, 1);
    row.progress.setPosition(trackX + ORDER_PROGRESS_BAR_WIDTH, row.rowCenter + layout.labelDy);
    row.progress.setText(`${clampedProgress}/${order.need}`);
    row.progress.setFontSize(10);

    const plan = this.orderMilestonePlan(order);
    const triggered = new Set(order.milestonesTriggered || []);
    row.milestoneNodes.forEach(({ node }, index) => {
      const milestone = plan[index];
      if (!milestone) {
        node.setVisible(false);
        return;
      }
      const nodeX = trackX + ORDER_PROGRESS_BAR_WIDTH * milestone.at;
      const reached = triggered.has(`${index}:${milestone.kind}`);
      node.setPosition(nodeX, trackY)
        .setFillStyle(reached ? 0xfff06a : 0x5a1020, 1)
        .setStrokeStyle(2, reached ? 0xffffff : 0xfff06a, 1)
        .setVisible(this.currentUiMode === "game");
    });
  }

  fitOrderLabel(row) {
    const progressLeft = row.progress.x - row.progress.width;
    const maxRight = Math.min(row.reward.x - row.reward.width / 2 - 10, progressLeft - 7);
  ...
  setOrderNearState(row, active) {
    ...
    row.progress.setPosition(row.progressTrack.x + ORDER_PROGRESS_BAR_WIDTH, row.rowCenter + layout.labelDy);
  ...
  async finishMove() {
    ...
    this.updateOrders();
    await this.triggerMainOrderMilestones();
    this.updateOrders();
  ...
  fulfillCompletedOrders() {
    ...
    this.orders = this.orders.map((order, index) => {
      ...
      const baseFinalMult = paysCoins ? Math.max(1, Math.round(roll.mult * goldMult * boost.mult)) : 0;
      const upgrade = paysCoins ? this.rollMainOrderUpgrade(order) : { kind: "none", label: "", mult: 1 };
      const finalMult = paysCoins ? Math.max(1, Math.round(baseFinalMult * upgrade.mult)) : 0;
      ...
      refreshedOrders += 1;
      if (isFreeOrder && MATH_CONFIG.freeOrdersRefreshOnComplete === false) {
